...
module.exports = Requirement.clss('PackageLoader', function (def) {
	...
	def.loadFile = function (name, callback) {
		if (!def.loadFile.FileLoader) {
			def.loadFile.FileLoader = FileLoader = require('./FileLoader');
		}

		new FileLoader(this.dependencyList, name, this.dir, this.finish.bind(this)).process(callback);
